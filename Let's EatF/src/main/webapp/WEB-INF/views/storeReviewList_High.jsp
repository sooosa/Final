...
<h3 style="text-align : center"><br><br>${store.sName} 리뷰 목록</h3><br>
	...
	<select onchange="sort()" name="reviewSort" id="reviewSort">
		...
		<option value="평점 높은순" selected>평점 높은순</option>
		<option value="평점 낮은순">평점 낮은순</option>
	</select>
	

	<!-- 해당 업체 리뷰 목록 -->
	<c:forEach var="storeRListHigh" items="${storeRListHigh}">
		<div class="reviewdiv">
			<h2>${storeRListHigh.rsName}</h2> 
			<div><img src="resources/img/${storeRListHigh.rStar}.png" class="star">&nbsp;&nbsp;<b>${storeRListHigh.rStar}</b></div><br>
			<div style="word-break:break-all">${storeRListHigh.rContents}</div><br>
			<span style="color:#8F8F8F">${storeRListHigh.rBook}&nbsp;&nbsp;|&nbsp;&nbsp;${storeRListHigh.rmId}</span> 
			<span style="position: absolute; left : 65%;"><button onclick="storeReviewDel('${storeRListHigh.rmId}','${storeRListHigh.rCode}')">삭제</button></span>
		</div><br>
		
		<input type="hidden" id="loginId" value="${sessionScope.loginId}">
		<input type="hidden" id="storeCode" value="${storeRListHigh.rsCode}">
